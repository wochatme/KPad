...
#include "uiview/widget/root_view.h"
#include "uiview/widget/widget.h"

#include "dragged_tab_controller.h"
...
namespace
{
    ...
    class ResetDraggingStateDelegate
        : public view::BoundsAnimator::OwnedAnimationDelegate
    {
    public:
...
private:
    ...
    void HighlightCloseButton()
    {
        ...
        view::Widget* widget = tabstrip_->GetWidget();
...
void BaseTabStrip::ContinueDrag(const view::MouseEvent& event)
{
    ...
    if (drag_controller_.get())
    {
        ...
        if (drag_controller_->started_drag() && !started_drag)
        {
            ...
            static_cast<view::internal::RootView*>(
                GetWidget()->GetRootView())->SetMouseHandler(this);
        }
    }
}
...
const view::View* BaseTabStrip::GetViewByID(int view_id) const
{
    if (tab_count() > 0)
    {
...
bool BaseTabStrip::OnMouseDragged(const view::MouseEvent& event)
{
    if (drag_controller_.get())
    {
        drag_controller_->Drag();
...
void BaseTabStrip::StartRemoveTabAnimation(int model_index)
{
    ...
    bounds_animator_.SetAnimationDelegate(tab,
        new RemoveTabDelegate(this, tab), true);
}
...
{
    view::View* view = GetEventHandlerForPoint(local_point);
    if (!view)
    {
