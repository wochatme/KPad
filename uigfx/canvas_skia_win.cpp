...
namespace
{
    ...
    void DoDrawText(HDC hdc, const string16& text, RECT* text_bounds, int flags)
    {
    ...
    void FadeBitmapRect(SkDevice& bmp_device,
        const gfx::Rect& rect, bool fade_to_right)
    {
        ...
        const gfx::Rect& gfx_draw_rect)
    {
        ...
        for (int y = draw_rect.top; y < draw_rect.bottom; ++y)
        {
            for (int x = draw_rect.left; x < draw_rect.right; ++x)
            {
                ...
                BYTE alpha = 0xFF - gfx::GetLuminanceForColor(color);
                *bmp.getAddr32(x, y) = SkPreMultiplyColor(SkColorSetARGB(alpha,
...
namespace gfx
{
        ...
        int flags)
    {
        ...
        // opaque.
        skia::MakeOpaque(this, clip.fLeft, clip.fTop, clip.width(), clip.height());
    }
        ...
        const gfx::Rect& display_rect)
    {
        ...
        case TruncateFadeHeadAndTail:
            ...
            string16 clipped_head_string = text.substr(0,
                desired_characters_to_truncate_from_head);
            int clipped_width;
            int clipped_height;
            SizeStringInt(clipped_head_string, font, &clipped_width,
                &clipped_height, flags);
        ...
        DrawStringInt(text, font, color,
            ...
            text_rect.width(), text_rect.height(), flags);
        restore();
        restore();
    }
