...
namespace gfx
{
        ...
        const ScrollbarTrackExtraParams& extra) const
    {
        ...
        {
            SkPaint paint;
            RECT align_rect = gfx::Rect(extra.track_x, extra.track_y,
                extra.track_width, extra.track_height).ToRECT();
        ...
        const MenuArrowExtraParams& extra) const
    {
        ...
        if (handle && draw_theme_)
        {
            ...
            {
                return draw_theme_(handle, hdc, MENU_POPUPSUBMENU, state_id,
                    &rect_win, NULL);
            }
            else
            {
        ...
        return PaintFrameControl(hdc, rect, DFC_MENU, pfc_state,
        ...
        const gfx::Rect& rect) const
    {
        ...
        if (handle && draw_theme_)
        {
            return draw_theme_(handle, hdc, MENU_POPUPCHECKBACKGROUND,
                state_id, &rect_win, NULL);
        }
        ...
        const MenuCheckExtraParams& extra) const
    {
        ...
        if (handle && draw_theme_)
        {
            return draw_theme_(handle, hdc, MENU_POPUPCHECK, state_id,
                &rect_win, NULL);
        }
        ...
        const gfx::Rect& rect) const
    {
        ...
        if (handle && draw_theme_)
        {
            return draw_theme_(handle, hdc, MENU_POPUPGUTTER, MPI_NORMAL,
                &rect_win, NULL);
        }
        return E_NOTIMPL;
    }
        ...
        const MenuListExtraParams& extra) const
    {
        ...
        if (handle && draw_theme_)
        {
            return draw_theme_(handle, hdc, CP_DROPDOWNBUTTON, state_id,
                &rect_win, NULL);
        }
        ...
        const MenuSeparatorExtraParams& extra) const
    {
        ...
        if (handle && draw_theme_)
        {
            ...
            --rect_win.top;
            --rect_win.bottom;
            return draw_theme_(handle, hdc, MENU_POPUPSEPARATOR, MPI_NORMAL,
                &rect_win, NULL);
        }
        ...
        const TrackbarExtraParams& extra) const
    {
        ...
        {
            channel_rect.top += ((channel_rect.bottom -
                channel_rect.top - channel_thickness) / 2);
            channel_rect.bottom = channel_rect.top + channel_thickness;
        }
        else if (part_id == TKP_TRACKVERT)
        {
            channel_rect.left += ((channel_rect.right -
                channel_rect.left - channel_thickness) / 2);
        ...
        if (handle && draw_theme_ && draw_theme_ex_)
        {
            ...
            if (extra.determinate)
            {
                ...
                else
                {
                    ...
                    draw_theme_(handle, hdc, PP_MOVEOVERLAY, 0, &overlay_rect,
                        &value_rect);
                }
            }
            else
        ...
        const TextFieldExtraParams& extra) const
    {
        ...
        return PaintTextField(hdc, part_id, state_id, extra.classic_state,
            &rect_win, skia::SkColorToCOLORREF(extra.background_color),
        ...
        SIZE* size) const
    {
        ...
        {
            return get_theme_part_size_(handle, hdc, part_id, state_id, rect,
                ts, size);
        }
