#include "interpolated_transform.h"
...
#include "uibase/animation/tween.h"
...
namespace ui
{
    ...
    InterpolatedTransform::InterpolatedTransform()
        : start_time_(0.0f), end_time_(1.0f) {}
    ...
    inline float InterpolatedTransform::ValueBetween(float time,
        float start_value, float end_value) const
    {
        // can't handle NaN
    ...
    gfx::Transform InterpolatedTranslation::InterpolateButDoNotCompose(float t) const
    {
        gfx::Transform result;
        // TODO(vollick) 3d xforms.
    ...
    gfx::Transform InterpolatedConstantTransform::InterpolateButDoNotCompose(
        float t) const
    {
        return transform_;
    }
    ...
    gfx::Transform InterpolatedTransformAboutPivot::InterpolateButDoNotCompose(
        float t) const
    {
        if (transform_.get())
        {
    ...
    gfx::Transform InterpolatedTRSTransform::InterpolateButDoNotCompose(float t) const
    {
        if (transform_.get())
        {
            return transform_->Interpolate(t);
